...
class LateFusionModel(Module):
    ...
    def forward(self, batch_in: dict):
        batch_in = {x: batch_in[x].to(next(self.parameters()).device) for x in batch_in}
        bert_output = self._bert(input_ids=batch_in['bert_input_id'].squeeze(),
                                attention_mask=batch_in['bert_attention_mask'].squeeze())

        # Check if bert_output is a tuple and take the first element (logits) if so
        if isinstance(bert_output, tuple):
            cls_vector = bert_output[0][:, 0, :]  # Take the first token (CLS token) from the logits
        else:
            cls_vector = bert_output[:, 0, :]  # Directly take the first token (CLS token) if not a tuple

        resnet_feature = self._resnet(batch_in['image'])
        combined_features = torch.cat((cls_vector, resnet_feature), dim=1)
